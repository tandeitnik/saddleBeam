...
def gradient_x(x,y,z,t):
    ...
    grad_x = (4* np.exp(-((2* (x**2 + y**2))/(w0**2* (1 + z**2/zr**2))))*P *((4* np.sqrt(2)*B *np.cos(2* np.arctan2(z,zr))* (2 *x))/(w0**2* (1 + z**2/zr**2)) + (16* B**2 *(2* x)* ((x**2 - y**2)))/(w0**4 *(1 + z**2/zr**2)**2)))/((1 + 2 *B**2)* c* np.pi* w0**2 *(1 + z**2/zr**2)* e0) - (16* np.exp(-((2 *(x**2 + y**2))/(w0**2 *(1 + z**2/zr**2))))*P* x* (1 + (4* np.sqrt(2)*B* np.cos(2* np.arctan2(z,zr))* ((x**2 - y**2)))/(w0**2 *(1 + z**2/zr**2)) +(8* B**2 *((x**2 - y**2))**2)/(w0**4 *(1 + z**2/zr**2)**2)))/((1 + 2 *B**2) *c* np.pi *w0**4 *(1 + z**2/zr**2)**2* e0)
                                     
    return grad_x
@njit(fastmath=True)
def gradient_y(x,y,z,t):
    
    grad_y =  (4 *np.exp(-1*((2*(x**2 + y**2))/(w0**2*(1 + z**2/zr**2))))*P* ((4* np.sqrt(2)*B* np.cos(2*np.arctan2(z,zr))*(-2* y))/(w0**2*(1 + z**2/zr**2)) + (16* B**2*(-2* y)*((x**2 - y**2)))/(w0**4 *(1 + z**2/zr**2)**2)))/((1 + 2*B**2)* c* np.pi *w0**2*(1 + z**2/zr**2)* e0) - (16* np.exp(-((2*(x**2 + y**2))/(w0**2*(1 + z**2/zr**2))))*P* y* (1 + (4* np.sqrt(2)*B* np.cos(2*np.arctan2(z,zr))*((x**2 - y**2)))/(w0**2*(1 + z**2/zr**2)) + (8 *B**2*((x**2 - y**2))**2)/( w0**4* (1 + z**2/zr**2)**2)))/((1 + 2*B**2)* c* np.pi* w0**4 *(1 + z**2/zr**2)**2* e0)
                                    
    return grad_y
@njit(fastmath=True)
def gradient_z(x,y,z,t):
    
    grad_z =  (16* np.exp(-((2 *(x**2 + y**2))/(w0**2 *(1 + z**2/zr**2))))*P* (x**2 + y**2) *z* (1 + (4 *np.sqrt(2)*B* np.cos(2*np.arctan2(z,zr))*((x**2 - y**2)))/(w0**2*(1 + z**2/zr**2)) + (8*B**2*((x**2 - y**2))**2)/(w0**4 *(1 + z**2/zr**2)**2)))/((1 + 2*B**2)* c* np.pi*w0**4 *(1 + z**2/zr**2)**3*zr**2*e0) - (8*np.exp(-((2*(x**2 + y**2))/(w0**2*(1 + z**2/zr**2))))*P* z* (1 + (4 *np.sqrt(2)*B* np.cos(2*np.arctan2(z,zr))*((x**2 - y**2))/(w0**2*(1 + z**2/zr**2)) + (8*B**2*((x**2 - y**2))**2)/( w0**4 *(1 + z**2/zr**2)**2))))/((1 + 2*B**2)* c* np.pi*w0**2*(1 + z**2/zr**2)**2 *zr**2*e0) + (4 *np.exp(-((2*(x**2 + y**2))/(w0**2*(1 + z**2/zr**2))))*P* (-((8*np.sqrt(2)*B* z* np.cos(2*np.arctan2(z,zr))*((x**2 - y**2)))/(w0**2*(1 + z**2/zr**2)**2* zr**2)) - (32*B**2* z* ((x**2 - y**2))**2)/(w0**4 *(1 + z**2/zr**2)**3*zr**2) - ( 8 *np.sqrt(2)*B* ((x**2 - y**2))*np.sin(2*np.arctan2(z,zr)))/(w0**2*(1 + z**2/zr**2)**2* zr)))/((1 + 2*B**2)* c* np.pi*w0**2*(1 + z**2/zr**2)*e0)
...
def intensity(x,y,z,t):
    ...
    coef_0 = 1
    coef_2 = 4*np.sqrt(2)*B*np.cos(2*phi+2*0*t)*np.cos(2*chi)/w**2
    coef_4 = 8*B**2*np.cos(2*phi+2*0*t)**2/w**4
...
def ode_x(x,y,z,v_x,v_y,t):
    ...
    ode_1 = v_x
    ode_2 = gradient_x(x,y,z,t)*np.real(alpha_rad)/(4*mass_particle) -gamma*v_x + gamma*Omega*y + 2*Omega*v_y + Omega**2*x
...
def ode_y(x,y,z,v_x,v_y,t):
    ...
    ode_1 = v_y
    ode_2 = gradient_y(x,y,z,t)*np.real(alpha_rad)/(4*mass_particle) -gamma*v_y - gamma*Omega*x - 2*Omega*v_x + Omega**2*y
...
# plt.scatter(Omega_list/stabilityCriteria,rhoSqrtVarianceMean/wl, marker = 'd', label = 'lin. dynamics')
...
# x = np.linspace(-n*w0,n*w0,res)
# y = np.linspace(-n*w0,n*w0,res)
# z_eq = 3*wl**4*e0*np.real(alpha_rad)/(16*np.pi**3*np.abs(alpha_rad)**2) -0.5*np.sqrt((3*wl**4*e0*np.real(alpha_rad)/(8*np.pi**3*np.abs(alpha_rad)**2))**2 -4*zr**2)

# X, Y = np.meshgrid(x,y)
# theta = 0
# def electricFieldSquared(x,y,z):
...
#     pre = (2*P/(c*e0))*(1/(1+2*B**2))*(2/np.pi)*(1/w**2)*np.exp(-2*r**2/w**2)
#     coef_0 = 1
#     coef_2 = 4*np.sqrt(2)*B*np.cos(2*phi+2*theta)*np.cos(2*chi)/w**2
#     coef_4 = 8*B**2*np.cos(2*phi+2*theta)**2/w**4
    
#     eSq = pre*(coef_0 + coef_2*r**2 + coef_4*r**4)
...
# plt.imshow(electricFieldSquared(X,Y,z_eq), cmap = 'jet',origin='lower', alpha = 1, extent=[-n*w0,n*w0,-n*w0,n*w0])
