...
def ode_x(x,y,z,v_x,v_y,t):
    ...
    ode_1 = v_x
    ode_2 = np.sign(kx)*w_x**2*x -gamma*v_x + gamma*Omega*y + 2*Omega*v_y + Omega**2*x
...
def ode_y(x,y,z,v_x,v_y,t):
    ...
    ode_1 = v_y
    ode_2 = np.sign(ky)*w_y**2*y -gamma*v_y - gamma*Omega*x - 2*Omega*v_x + Omega**2*y
...
def ode_z(x,y,z,v_z,t):
    ...
    ode_1 = v_z
    ode_2 = np.sign(kz)*w_z**2*z -gamma*v_z  + intensity(x,y,z,t)*sigma_ext/(c*mass_particle)
