...
dt_simulation = 1e-9 #simulation time step, s
dt_simulation #sampling of the data
...
@njit(fastmath=True)
def gradient_x(x,y,z,t):

    grad_x = (4* np.exp(-((2* (x**2 + y**2))/(w0**2* (1 + z**2/zr**2)))) *P *((4* np.sqrt(2)*B *np.cos(2* np.arctan2(z,zr))* (2 *x* np.cos(2 *Omega*t) - 2* y* np.sin(2 *Omega*t)))/(w0**2* (1 + z**2/zr**2)) + (16* B**2 *(2* x* np.cos(2* Omega*t) - 2* y* np.sin(2 *Omega*t))* ((x**2 - y**2) *np.cos(2* Omega*t) - 2*x*y *np.sin(2 *Omega*t)))/(w0**4 *(1 + z**2/zr**2)**2)))/((1 + 2 *B**2)* c* np.pi* w0**2 *(1 + z**2/zr**2)* e0) - (16* np.exp(-((2 *(x**2 + y**2))/(w0**2 *(1 + z**2/zr**2))))*P* x* (1 + (4* np.sqrt(2)*B* np.cos(2* np.arctan2(z,zr))* ((x**2 - y**2) *np.cos(2 *Omega*t) - 2*x*y* np.sin(2* Omega*t)))/(w0**2 *(1 + z**2/zr**2)) + (8* B**2 *((x**2 - y**2)* np.cos(2 *Omega*t) - 2*x*y* np.sin(2* Omega*t))**2)/(w0**4 *(1 + z**2/zr**2)**2)))/((1 + 2 *B**2) *c* np.pi *w0**4 *(1 + z**2/zr**2)**2* e0)
                         
                                  
    return grad_x
@njit(fastmath=True)
def gradient_y(x,y,z,t):
    
    grad_y =  (4 *np.exp(-1*((2*(x**2 + y**2))/(w0**2*(1 + z**2/zr**2))))*P* ((4* np.sqrt(2)*B* np.cos(2*np.arctan2(z,zr))*(-2* y* np.cos(2*Omega*t) - 2 *x* np.sin(2*Omega*t)))/(w0**2*(1 + z**2/zr**2)) + (16* B**2*(-2* y* np.cos(2*Omega*t) - 2 *x* np.sin(2*Omega*t))*((x**2 - y**2) *np.cos(2*Omega*t) - 2*x*y*np.sin(2*Omega*t)))/(w0**4 *(1 + z**2/zr**2)**2)))/((1 + 2*B**2)* c* np.pi *w0**2*(1 + z**2/zr**2)* e0) - (16* np.exp(-((2*(x**2 + y**2))/(w0**2*(1 + z**2/zr**2))))*P* y* (1 + (4* np.sqrt(2)*B* np.cos(2*np.arctan2(z,zr))*((x**2 - y**2) *np.cos(2*Omega*t) - 2*x*y*np.sin(2*Omega*t)))/(w0**2*(1 + z**2/zr**2)) + (8 *B**2*((x**2 - y**2) *np.cos(2*Omega*t) - 2*x*y*np.sin(2*Omega*t))**2)/(w0**4* (1 + z**2/zr**2)**2)))/((1 + 2*B**2)* c* np.pi* w0**4 *(1 + z**2/zr**2)**2* e0)
                                    
    return grad_y
@njit(fastmath=True)
def gradient_z(x,y,z,t):
    
    grad_z =  (16* np.exp(-((2 *(x**2 + y**2))/(w0**2 *(1 + z**2/zr**2))))*P* (x**2 + y**2) *z* (1 + (4 *np.sqrt(2)*B* np.cos(2*np.arctan2(z,zr))*((x**2 - y**2)*np.cos(2 *Omega*t) - 2* x* y* np.sin(2 *Omega*t)))/(w0**2*(1 + z**2/zr**2)) + (8*B**2*((x**2 - y**2)*np.cos(2 *Omega*t) - 2* x* y* np.sin(2 *Omega*t))**2)/(w0**4 *(1 + z**2/zr**2)**2)))/((1 + 2*B**2)* c* np.pi*w0**4 *(1 + z**2/zr**2)**3*zr**2*e0) - (8*np.exp(-((2*(x**2 + y**2))/(w0**2*(1 + z**2/zr**2))))*P* z* (1 + (4 *np.sqrt(2)*B* np.cos(2*np.arctan2(z,zr))*((x**2 - y**2)*np.cos(2 *Omega*t) - 2* x* y* np.sin(2 *Omega*t)))/(w0**2*(1 + z**2/zr**2)) + (8*B**2*((x**2 - y**2)*np.cos(2 *Omega*t) - 2* x* y* np.sin(2 *Omega*t))**2)/( w0**4 *(1 + z**2/zr**2)**2)))/((1 + 2*B**2)* c* np.pi*w0**2*(1 + z**2/zr**2)**2 *zr**2*e0) + ( 4 *np.exp(-((2*(x**2 + y**2))/(w0**2*(1 + z**2/zr**2))))*P* (-((8*np.sqrt(2)*B* z* np.cos(2*np.arctan2(z,zr))*((x**2 - y**2)*np.cos(2 *Omega*t) - 2* x* y* np.sin(2 *Omega*t)))/(w0**2*(1 + z**2/zr**2)**2* zr**2)) - (32*B**2* z* ((x**2 - y**2)*np.cos(2 *Omega*t) - 2* x* y* np.sin(2 *Omega*t))**2)/(w0**4 *(1 + z**2/zr**2)**3*zr**2) - (8 *np.sqrt(2)*B* ((x**2 - y**2)*np.cos(2 *Omega*t) - 2* x* y* np.sin(2 *Omega*t))*np.sin(2*np.arctan2(z,zr)))/(w0**2*(1 + z**2/zr**2)**2* zr)))/((1 + 2*B**2)* c* np.pi*w0**2*(1 + z**2/zr**2)*e0)
...
def ode_x(x,y,z,v_x,v_y,t):
    ...
    ode_1 = v_x
    ode_2 = gradient_x(x,y,z,t)*np.real(alpha_rad)/(4*mass_particle) -gamma*v_x
...
def ode_y(x,y,z,v_x,v_y,t):
    ...
    ode_1 = v_y
    ode_2 = gradient_y(x,y,z,t)*np.real(alpha_rad)/(4*mass_particle) -gamma*v_y
...
def ode_z(x,y,z,v_z,t):
    ...
    ode_1 = v_z
    ode_2 = gradient_z(x,y,z,t)*np.real(alpha_rad)/(4*mass_particle) -gamma*v_z  + intensity(x,y,z,t)*sigma_ext/(c*mass_particle)
...
# plt.scatter(Omega_list/stabilityCriteria,np.sqrt(rhoVarianceMean)/wl, marker = 'd', label = 'full dynamics')
...
# indices = [[0,0],[2,0],[-2,0]] #list the indices of the modes present in the beam
# coef = np.array([A,B,B]) #zera o meio

# res = 200

# # x = np.linspace(np.min(pos_x),np.max(pos_x),res)
# # y = np.linspace(np.min(pos_y),np.max(pos_y),res)

# x = np.linspace(-2*w0,2*w0,res)
# y = np.linspace(-2*w0,2*w0,res)

# #plt.imshow(np.abs(beamConstructor(basis,indices,coef,x,y,0,w0,wl))**2, cmap = 'jet',origin='lower', alpha = 1, extent=[np.min(pos_x),np.max(pos_x),np.min(pos_y),np.max(pos_y)])
# plt.imshow(np.abs(beamConstructor(basis,indices,coef,x,y,0,w0,wl))**2, cmap = 'jet',origin='lower', alpha = 1, extent=[-2*w0,2*w0,-2*w0,2*w0])
# plt.plot(pos_x,pos_y,zorder=1)
# plt.scatter(pos_x[0],pos_y[0], c ='red', label = 'start',zorder=2)
# plt.scatter(pos_x[-1],pos_y[-1], c ='pink', label = 'end',zorder=2)
# plt.scatter(0,0, c ='black',label = 'origin',zorder=2)
# plt.xlim(-2*w0,2*w0)
# plt.ylim(-2*w0,2*w0)
# plt.legend()
